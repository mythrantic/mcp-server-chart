 ...
 * @returns {Promise<{dataUrl: string, imageUrl?: string, filename?: string}>} The generated chart data.
...
export async function generateChartUrl(
...
): Promise<{ dataUrl: string; imageUrl?: string; filename?: string }> {
  ...
  const { success, errorMessage, resultObj, imageUrl, filename } = response.data;
