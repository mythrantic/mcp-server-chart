...
export async function callTool(tool: string, args: object = {}) {
  ...
  try {
    // Extract includeResultData flag from args (defaults to false)
    const { includeResultData = false, ...chartArgs } = args as { includeResultData?: boolean; [key: string]: any };
    ...
    if (schema) {
      ...
      const result = z.object(schema).safeParse(chartArgs);
      if (!result.success) {
    ...
    if (isMapChartTool) {
      ...
      const { metadata, ...result } = await generateMap(tool, chartArgs);
      return result;
    }

    const { dataUrl, imageUrl, filename } = await generateChartUrl(chartType, chartArgs);
    logger.info(`Generated chart - Data URL length: ${dataUrl.length}, Image URL: ${imageUrl || 'N/A'}, includeResultData: ${includeResultData}`);

    // Build content array - only include dataUrl if includeResultData is true
    const content = [];
    ...
    return {
      ...
      _meta: {
        description:
          includeResultData 
            ? "Chart spec and configuration included. The imageUrl can be used to display the chart."
            : "Chart generated successfully. Use the imageUrl to display the chart. Set includeResultData=true to get the full chart specification.",
        spec: { type: chartType, ...chartArgs },
        imageUrl,
        filename,
      },
    };
